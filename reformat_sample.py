...
if __name__ == '__main__':
    with open('result/sample.md', 'w') as mdfile:
        mdfile.write('# Samples\n\n')

        with open('result/sample.csv') as csvfile:
            samples = csv.reader(csvfile)

            sample_num = 1
            for sample_row in samples:
                mdfile.write('## ' + str(sample_num) +
                             '. Sample ' + sample_row[0] + '\n\n')
                ...
                mdfile.write('```c++\n')
                comment = sample_row[2].replace(
                    '[comma]', ',').replace('[enter]', '\n')
                mdfile.write(comment)
                mdfile.write('\n```\n\n')

                mdfile.write('```c++\n')
                code = sample_row[5].replace(
                    '[comma]', ',').replace('[enter]', '\n')
                mdfile.write(code)
                mdfile.write('\n```\n\n')
